...
function storeFromFileName(fileName: string) {
  return fileName.replace(/\.(xlsx|xls)$/i, "").replace(/\d{4}[-年.]?\d{1,2}[-月.]?\d{0,2}[日]?/g, "").replace(/报表|日报|月报|工资表|库存表|核对表/g, "").replace(/[-_ ]+/g, " ").trim() || fileName.replace(/\.(xlsx|xls)$/i, "");
}
...
function parseProductFile(workbook: XLSX.WorkBook, fileName: string, storeName: string): BatchResult | null {
  ...
  return { fileName, storeName, kind: "product", rowCount, amount: round2(amount), issueCount: issues.length, issues };
}
...
function parseStructuredFile(workbook: XLSX.WorkBook, fileName: string, storeName: string): BatchResult {
  ...
  return { fileName, storeName, kind, rowCount, amount: round2(amount), issueCount: issues.length, issues };
}
...
async function analyzeFile(file: File): Promise<BatchResult> {
  ...
  const product = parseProductFile(workbook, fileName, storeName);
  if (product) return product;
  return parseStructuredFile(workbook, fileName, storeName);
}

function exportBatch(results: BatchResult[]) {
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(results.map((item, index) => ({
    序号: index + 1,
    门店: item.storeName,
    文件: item.fileName,
    类型: item.kind === "product" ? "商品表" : item.kind === "payroll" ? "工资表" : "通用表",
    数据行: item.rowCount,
    金额合计: item.amount,
    异常数: item.issueCount,
    状态: item.issueCount ? "需检查" : "正常"
  }))), "全部门店总览");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(results.flatMap((item) => item.issues).map((issue, index) => ({
    序号: index + 1,
    门店: issue.storeName,
    文件: issue.fileName,
    类型: issue.type,
...
export default function MultiFileSmartUploadPanel() {
  ...
  async function handleFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    setLoading(true);
    try {
      const next = await Promise.all(files.map(analyzeFile));
      setResults(next);
    } finally {
      setLoading(false);
      event.target.value = "";
  ...
  return <section className="mx-auto mb-6 max-w-7xl px-4 md:px-8">
    <div className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-900">智能上传：单个或多个 Excel</h2>
          <p className="mt-1 text-sm text-slate-500">老板批量看门店时，一次选择多个门店表格；只查一个表时，也可以只选一个。</p>
        </div>
      ...
      {results.length ? <>
        ...
        <div className="mt-5 overflow-auto rounded-xl border">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-left"><tr>{["门店", "文件", "类型", "数据行", "金额合计", "异常", "状态"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead>
            <tbody>{results.map((item) => <tr key={item.fileName} className="border-t">
              <td className="p-3">{item.storeName}</td>
              <td className="p-3">{item.fileName}</td>
              <td className="p-3">{item.kind === "product" ? "商品表" : item.kind === "payroll" ? "工资表" : "通用表"}</td>
              <td className="p-3">{item.rowCount}</td>
              <td className="p-3">¥{money(item.amount)}</td>
              <td className="p-3">{item.issueCount}</td>
              <td className="p-3">{item.issueCount ? "需检查" : "正常"}</td>
            </tr>)}</tbody>
          </table>
        </div>

        {totalIssues ? <div className="mt-5 rounded-xl bg-red-50 p-4 text-sm text-red-800">
          <div className="font-bold">异常预览</div>
          <ul className="mt-2 space-y-1">{results.flatMap((item) => item.issues).slice(0, 10).map((issue, index) => <li key={index}>• {issue.storeName}：{issue.message}</li>)}</ul>
        </div> : null}
