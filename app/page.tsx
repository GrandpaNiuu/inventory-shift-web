...
function makeWriteBack(row: StockRow, colIndex: number, before: number, after: number): WriteBackTarget {
  ...
  return { key: rowCellKey(row, colIndex), date: row.date, sheetName: row.sheetName, rowIndex: row.rowIndex, rowNumber: row.rowNumber, colIndex, cell, columnName: columnName(colIndex), category: row.category, name: row.name, price: row.price, before, after: round2(after) };
}
...
export default function Home() {
  ...
  const handovers = useMemo<HandoverIssue[]>(() => {
    ...
    rows.forEach((row) => { const key = `${row.category}__${row.name}__${row.price}`; const list = byProduct.get(key) ?? []; list.push(row); byProduct.set(key, list); });
  ...
  function correctedExpectedEndingStock(row: StockRow) { const correction = corrections[rowCellKey(row, NIGHT_STOCK_COLUMN)]; return correction ? round2(correction.after - row.nightSold) : row.endingStock; }
  function addOnsiteCorrection(row: StockRow, actualEndingStock: number) { const fixedNightStock = round2(actualEndingStock + row.nightSold); const target = makeWriteBack(row, NIGHT_STOCK_COLUMN, row.nightStock, fixedNightStock); setCorrections((old) => setCorrection(target, "按现场实点反推夜班库存，写回H列夜班库存", old)); setCountInput((old) => ({ ...old, [countKey(row)]: String(actualEndingStock) })); }
  ...
  function undoCorrection(row: StockRow) { undoCorrectionByKey(rowCellKey(row, NIGHT_STOCK_COLUMN)); }
  ...
  function exportSalesSummaryReport() {
    ...
    const aoa: Array<Array<string | number>> = [
      ...
      ...productSummary.map((item, index) => [index + 1, item.category, item.name, round2(item.price), round2(item.totalSold), round2(item.totalAmount)]),
  ...
  function exportExceptionReport() {
    ...
    const aoa: Array<Array<string | number>> = [
      ...
      ...handovers.map((item, index) => [index + 1, item.type, item.category, item.date, item.nextDate ?? "", item.name, item.expected, item.actual, item.diff, writeBackLocation(item), item.note]),
    ...
    for (let row = 4; row < totalRow; row += 1) { setNumFormat(sheet, row, 6, "0.00"); setNumFormat(sheet, row, 7, "0.00"); setNumFormat(sheet, row, 8, "0.00"); }
    const reportWorkbook = XLSX.utils.book_new();
