...
function extractDate(sheetName: string, title: unknown, fallbackIndex: number) {
  ...
  const match = raw.match(/(20\d{2}|19\d{2})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日?/);
  if (match) return `${match[1]}-${match[2].padStart(2, "0")}-${match[3].padStart(2, "0")}`;
  const sheetDay = sheetName.match(/^(?:Sheet)?(\d{1,2})$/i);
  const day = sheetDay ? Number(sheetDay[1]) : fallbackIndex + 1;
...
function parseWorkbook(workbook: XLSX.WorkBook) {
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    for (let i = 3; i < Math.min(grid.length, 90); i += 1) {
      ...
      const purchase = n(row[3]);
      const morningStock = n(row[4]);
      const morningSold = n(row[5]);
      const morningAmount = n(row[6]) || round2(morningSold * price);
      const nightStock = n(row[7]);
      const nightSold = n(row[8]);
      const nightAmount = n(row[9]) || round2(nightSold * price);
      const totalAmount = n(row[10]) || round2(morningAmount + nightAmount);
      ...
      parsed.push({
        ...
        nightAmount,
        totalAmount,
        endingStock: round2(nightStock - nightSold),
        sameDayExpectedNightStock,
        sameDayStockDiff: round2(nightStock - sameDayExpectedNightStock),
        morningAmountDiff: round2(morningAmount - morningSold * price),
        nightAmountDiff: round2(nightAmount - nightSold * price),
        totalAmountDiff: round2(totalAmount - morningAmount - nightAmount)
      });
    }
  });
...
function hasRowIssue(row: SmokeRow) {
  return Math.abs(row.sameDayStockDiff) > 0.01 || Math.abs(row.morningAmountDiff) > 0.01 || Math.abs(row.nightAmountDiff) > 0.01 || Math.abs(row.totalAmountDiff) > 0.01;
}
...
export default function Home() {
  ...
  const handovers = useMemo<HandoverIssue[]>(() => {
    ...
    rows.forEach((row) => {
      if (Math.abs(row.sameDayStockDiff) > 0.01) issues.push({ type: "早班交夜班", date: row.date, name: row.name, expected: row.sameDayExpectedNightStock, actual: row.nightStock, diff: row.sameDayStockDiff, note: "夜班库存应等于：早班库存 + 进货 - 早班售卖数量" });
  ...
  function exportAll() {
    downloadWorkbook("烟报自动核对汇总.xlsx", {
      ...
      原始明细: rows.map((row) => ({ 日期: row.date, Sheet: row.sheetName, 行号: row.rowNumber, 序号: row.no, 商品名称: row.name, 价格: row.price, 进货: row.purchase, 早班库存: row.morningStock, 早班售卖数量: row.morningSold, 早班售卖金额: row.morningAmount, 夜班库存: row.nightStock, 夜班售卖数量: row.nightSold, 夜班售卖金额: row.nightAmount, 合计金额: row.totalAmount, 夜班结束结存: row.endingStock }))
    });
  }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-6 rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">直接上传类似“6月手工烟报.xlsx”的文件。每个日期一个 sheet，左侧香烟表头为：商品名称、价格、进货、早班库存、早班售卖数量、夜班库存、夜班售卖数量。</p>
            </div>
          ...
          {fileName ? <p className="mt-3 text-sm text-slate-600">已导入：<b>{fileName}</b>，共解析 {rows.length} 条香烟明细。</p> : null}
        </section>
        ...
        {!rows.length ? (
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold">等待导入烟报</h2>
            <p className="mt-2 text-sm text-slate-600">请先上传你刚才发的那种手工烟报 Excel。导入后这里会自动显示汇总和异常。</p>
          </section>
        ) : null}
        ...
        {rows.length > 0 && tab === "overview" ? (
          <section className="space-y-6">
            <div className="grid gap-4 md:grid-cols-4">
              <Card title="日期数量" value={`${daily.length} 天`} desc={`最新日期 ${latestDate}`} />
              <Card title="香烟品规" value={`${productSummary.length} 个`} />
              <Card title="月销量" value={`${round2(totalSold)} 包`} />
              <Card title="月销售金额" value={`¥${money(totalAmount)}`} desc={`异常 ${handovers.length} 条`} />
            </div>
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <h2 className="text-lg font-bold">一键导出核对结果</h2>
                  <p className="mt-1 text-sm text-slate-500">导出后包含：每日汇总、商品月汇总、交接异常、原始明细。</p>
                </div>
                <button onClick={exportAll} className="rounded-xl bg-green-700 px-4 py-2 text-sm font-medium text-white">导出烟报汇总 Excel</button>
              </div>
            </div>
          </section>
        ) : null}

        {rows.length > 0 && tab === "daily" ? (
          <section className="rounded-2xl border bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold">每日烟报汇总</h2>
            <div className="table-scroll mt-4"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{["日期", "进货", "早班销量", "夜班销量", "总销量", "早班金额", "夜班金额", "总金额", "异常"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{daily.map((item) => <tr key={item.date} className="border-t"><td className="p-3">{item.date}</td><td className="p-3">{item.purchase}</td><td className="p-3">{item.morningSold}</td><td className="p-3">{item.nightSold}</td><td className="p-3">{item.totalSold}</td><td className="p-3">¥{money(item.morningAmount)}</td><td className="p-3">¥{money(item.nightAmount)}</td><td className="p-3">¥{money(item.totalAmount)}</td><td className="p-3">{item.abnormalCount ? <Badge tone="red">{item.abnormalCount} 条</Badge> : <Badge tone="green">正常</Badge>}</td></tr>)}</tbody></table></div>
          </section>
        ) : null}

        {rows.length > 0 && tab === "summary" ? (
          <section className="rounded-2xl border bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold">商品月汇总</h2>
            <div className="table-scroll mt-4"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{["商品", "价格", "月进货", "早班销量", "夜班销量", "月销量", "月销售金额", "月末结存"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{productSummary.map((item) => <tr key={`${item.name}-${item.price}`} className="border-t"><td className="p-3">{item.name}</td><td className="p-3">¥{money(item.price)}</td><td className="p-3">{item.purchase}</td><td className="p-3">{item.morningSold}</td><td className="p-3">{item.nightSold}</td><td className="p-3">{item.totalSold}</td><td className="p-3">¥{money(item.totalAmount)}</td><td className="p-3">{item.endingStock}</td></tr>)}</tbody></table></div>
          </section>
        ) : null}

        {rows.length > 0 && tab === "handover" ? (
          <section className="rounded-2xl border bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold">交接和金额异常</h2>
            <p className="mt-1 text-sm text-slate-500">检查：早班库存 + 进货 - 早班售卖 = 夜班库存；前一天夜班结束结存 = 第二天早班库存；金额 = 数量 × 价格。</p>
            <div className="table-scroll mt-4"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{["类型", "日期", "次日", "商品", "应为", "实际", "差异", "说明"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{handovers.map((item, index) => <tr key={`${item.type}-${item.date}-${item.name}-${index}`} className="border-t"><td className="p-3"><Badge tone="red">{item.type}</Badge></td><td className="p-3">{item.date}</td><td className="p-3">{item.nextDate ?? ""}</td><td className="p-3">{item.name}</td><td className="p-3">{item.expected}</td><td className="p-3">{item.actual}</td><td className="p-3">{item.diff}</td><td className="p-3">{item.note}</td></tr>)}</tbody></table></div>
          </section>
        ) : null}
