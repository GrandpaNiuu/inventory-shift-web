...
type Tab = "overview" | "detect" | "rules" | "daily" | "summary" | "handover" | "count" | "payment";
...
type StockRow = RawRow & { category: string };
type PaymentRecord = { date: string; sheetName: string; shift: PaymentShift; channel: string; amount: number };
type ParsedWorkbook = { rows: RawRow[]; payments: PaymentRecord[]; sheets: SheetPreview[] };
...
type Issue = { id: string; type: IssueType; date: string; nextDate?: string; category: string; name: string; expected: number; actual: number; diff: number; note: string; writeBack?: WriteBackTarget };
...
const tabs: Array<[Tab, string]> = [["overview", "总览"], ["detect", "识别预览"], ["rules", "分类规则"], ["daily", "每日报表"], ["summary", "商品总汇"], ["handover", "交接核对"], ["count", "现场盘点"], ["payment", "收款统计"]];
const DEFAULT_RULES: CategoryRule[] = [
  { id: "cigarette", name: "香烟(酒店库存)", keywords: "烟,香烟,卷烟,烟草,中华,芙蓉王,利群,玉溪,黄鹤楼,云烟,南京,双喜,红塔山,白沙,娇子,黄金叶,苏烟,泰山,七匹狼,中南海,牡丹,贵烟,真龙,钻石,煊赫门,万宝路,黄山,长白山,延安,兰州,宽窄,荷花,天子,红河,红金龙,金圣,人民大会堂,细支,中支,雨花石", includeSales: true, includeStockCheck: true, includeCount: true },
  { id: "drink", name: "饮料", keywords: "饮料,矿泉水,纯净水,可乐,雪碧,芬达,苏打水,气泡水,红牛,东鹏,脉动,外星人,农夫山泉,怡宝,百岁山,王老吉,加多宝,茶,冰红茶,绿茶,乌龙茶,咖啡,牛奶,酸奶,椰汁,果汁,柠檬茶,阿萨姆,海之言,娃哈哈,康师傅", includeSales: true, includeStockCheck: true, includeCount: false },
  { id: "lighter", name: "打火机", keywords: "打火机,火机,火柴,点烟器", includeSales: true, includeStockCheck: true, includeCount: false },
  { id: "misc", name: "杂项", keywords: "杂项,其他,小商品,日用品,百货,纸巾,扑克牌,牙刷,牙膏,剃须,充电器,数据线,雨伞,方便面,泡面,螺蛳粉,零食,口香糖", includeSales: true, includeStockCheck: false, includeCount: false }
];

const TITLE_STYLE = { font: { bold: true, sz: 16 }, alignment: { horizontal: "center", vertical: "center" } };
const HEADER_STYLE = { font: { bold: true, color: { rgb: "FFFFFF" } }, alignment: { horizontal: "center", vertical: "center" }, fill: { fgColor: { rgb: "1F2937" } } };
...
function countKey(row: StockRow) { return `${row.date}__${row.sheetName}__${row.rowNumber}__${row.name}__${row.price}`; }
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    groups.forEach((group) => {
      ...
      for (let i = group.headerRow + 1; i < grid.length; i += 1) {
        ...
        parsedRows.push({ date, sheetName, rowIndex: i, rowNumber: i + 1, sourceOrder: sheetIndex * 1_000_000 + group.startCol * 10_000 + i, rawCategory: categorize(`${name} ${currentCategory}`, rules) === UNKNOWN_CATEGORY ? currentCategory : categorize(`${name} ${currentCategory}`, rules), name, price, purchase, morningStock, morningSold, morningAmount, nightStock, nightSold, nightAmount, totalAmount, endingStock, sameDayExpectedNightStock, sameDayStockDiff: hasNightStock ? round2(nightStock - sameDayExpectedNightStock) : 0, morningAmountDiff: hasMorningSold || hasMorningAmount ? round2(morningAmount - morningSold * price) : 0, nightAmountDiff: hasNightSold || hasNightAmount ? round2(nightAmount - nightSold * price) : 0, totalAmountDiff: hasMorningAmount || hasNightAmount || hasTotalAmount ? round2(totalAmount - morningAmount - nightAmount) : 0, hasMorningStock, hasMorningSold, hasNightStock, hasNightSold, hasMorningAmount, hasNightAmount, hasTotalAmount, morningStockCol: group.morningStockCol, nightStockCol: group.nightStockCol });
        sheetRows += 1;
      }
    });
...
function setCellStyle(sheet: XLSX.WorkSheet, row: number, col: number, style: Record<string, unknown>) { const address = XLSX.utils.encode_cell({ r: row, c: col }); const cell = (sheet[address] ?? { t: "s", v: "" }) as CellWithStyle; cell.s = { ...(cell.s ?? {}), ...style }; sheet[address] = cell; }
...
export default function InventoryPageConfigurable() {
  ...
  const rows = useMemo<StockRow[]>(() => rawRows.map((row) => ({ ...row, category: categorize(`${row.name} ${row.rawCategory}`, rules) })), [rawRows, rules]);
  async function handleFile(file: File) { const nextWorkbook = XLSX.read(await file.arrayBuffer(), { type: "array", cellDates: true, cellFormula: true, cellNF: true, cellStyles: true }); const parsed = parseWorkbook(nextWorkbook, rules); setWorkbook(nextWorkbook); setRawRows(parsed.rows); setPaymentRecords(parsed.payments); setSheetPreview(parsed.sheets); setFileName(file.name); setCountInput({}); setCorrections({}); }
  ...
  const countCategoryNames = useMemo(() => new Set(rules.filter((rule) => rule.includeCount).map((rule) => rule.name)), [rules]);
  ...
  const countRows = useMemo(() => rows.filter((row) => row.date === reportProgressDate && countCategoryNames.has(row.category)).sort(byOriginalOrder), [rows, reportProgressDate, countCategoryNames]);
  ...
  const issues = useMemo<Issue[]>(() => { const out: Issue[] = []; stockCheckRows.forEach((row) => { if (row.hasNightStock && Math.abs(row.sameDayStockDiff) > 0.01) { const type: IssueType = row.morningSold > 0 && Math.abs(row.sameDayStockDiff - row.morningSold) < 0.01 ? "疑似未扣早班销量" : "早班交夜班"; const target = makeWriteBack(row, row.nightStockCol, row.nightStock, row.sameDayExpectedNightStock); out.push({ id: `${type}_${target.key}`, type, date: row.date, category: row.category, name: row.name, expected: row.sameDayExpectedNightStock, actual: row.nightStock, diff: row.sameDayStockDiff, note: type === "疑似未扣早班销量" ? "夜班库存比正确值多出的数量，正好等于早班销量。" : "夜班库存应等于：早班库存 + 进货 - 早班售卖数量。", writeBack: target }); } if (Math.abs(row.morningAmountDiff) > 0.01) out.push({ id: `金额核对_早班_${rowKey(row)}`, type: "金额核对", date: row.date, category: row.category, name: row.name, expected: round2(row.morningSold * row.price), actual: row.morningAmount, diff: row.morningAmountDiff, note: "早班金额不等于：早班售卖数量 × 价格。" }); if (Math.abs(row.nightAmountDiff) > 0.01) out.push({ id: `金额核对_夜班_${rowKey(row)}`, type: "金额核对", date: row.date, category: row.category, name: row.name, expected: round2(row.nightSold * row.price), actual: row.nightAmount, diff: row.nightAmountDiff, note: "夜班金额不等于：夜班售卖数量 × 价格。" }); }); const byProduct = new Map<string, StockRow[]>(); stockCheckRows.forEach((row) => byProduct.set(`${row.category}__${row.name}__${row.price}`, [...(byProduct.get(`${row.category}__${row.name}__${row.price}`) ?? []), row])); byProduct.forEach((list) => { const sorted = [...list].sort((a, b) => a.date.localeCompare(b.date)); for (let i = 0; i < sorted.length - 1; i += 1) { const current = sorted[i]; const next = sorted[i + 1]; if (!current.hasNightStock || !next.hasMorningStock) continue; const expected = round2(current.nightStock - current.nightSold); const diff = round2(next.morningStock - expected); if (Math.abs(diff) > 0.01) { const target = makeWriteBack(next, next.morningStockCol, next.morningStock, expected); out.push({ id: `次日接班_${target.key}_${current.date}`, type: "次日接班", date: current.date, nextDate: next.date, category: current.category, name: current.name, expected, actual: next.morningStock, diff, note: "次日早班库存应等于：前一日夜班库存 - 前一日夜班售卖数量。", writeBack: target }); } } }); return out.sort((a, b) => `${a.date}-${a.category}-${a.name}-${a.type}`.localeCompare(`${b.date}-${b.category}-${b.name}-${b.type}`)); }, [stockCheckRows]);
  ...
  function addOnsiteCorrection(row: StockRow, actualEndingStock: number) { const target = makeWriteBack(row, row.nightStockCol, row.nightStock, round2(actualEndingStock + row.nightSold)); setCorrection(target, "按现场实点反推夜班库存，写回夜班库存列"); setCountInput((old) => ({ ...old, [countKey(row)]: String(actualEndingStock) })); }
  ...
  function addRule() { setRules((old) => [...old, { id: `rule_${Date.now()}`, name: "新分类", keywords: "", includeSales: true, includeStockCheck: false, includeCount: false }]); }
  function removeRule(id: string) { setRules((old) => old.filter((rule) => rule.id !== id)); }

  function exportSalesSummaryReport() { if (!productSummary.length) return alert("请先上传原始报表 Excel"); const grouped = Array.from(productSummary.reduce((map, item) => { map.set(item.category, [...(map.get(item.category) ?? []), item]); return map; }, new Map<string, ProductSummary[]>()).entries()).sort(([, a], [, b]) => Math.min(...a.map((item) => item.sourceOrder)) - Math.min(...b.map((item) => item.sourceOrder))); const aoa: Array<Array<string | number>> = [["SY11 小商品销售汇总报表", "", "", "", "", ""], [`门店：${STORE_NAME}`, "", `营业日范围：${reportDateRange(daily)}`, "", `最后查询时间：${queryTime()}`, ""], ["", "", "", "", "", ""], ["序号", "商品分类", "商品名称", "平均价格", "数量", "金额"]]; let index = 1; grouped.forEach(([category, products]) => { aoa.push([`【${category}】`, "", "", "", "", ""]); products.sort((a, b) => a.sourceOrder - b.sourceOrder).forEach((item) => { aoa.push([index, item.category, item.name, round2(item.price), round2(item.totalSold), round2(item.totalAmount)]); index += 1; }); aoa.push([`${category}小计`, "", "", "", round2(products.reduce((sum, item) => sum + item.totalSold, 0)), round2(products.reduce((sum, item) => sum + item.totalAmount, 0))]); aoa.push(["", "", "", "", "", ""]); }); aoa.push(["全部总计", "", "", "", round2(productSummary.reduce((sum, item) => sum + item.totalSold, 0)), round2(productSummary.reduce((sum, item) => sum + item.totalAmount, 0))]); const sheet = XLSX.utils.aoa_to_sheet(aoa); sheet["!cols"] = [{ wch: 10 }, { wch: 20 }, { wch: 34 }, { wch: 12 }, { wch: 12 }, { wch: 14 }]; sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }]; styleRow(sheet, 0, 6, TITLE_STYLE); styleRow(sheet, 3, 6, HEADER_STYLE); styleRow(sheet, aoa.length - 1, 6, TOTAL_STYLE); for (let row = 4; row < aoa.length; row += 1) { setNumFormat(sheet, row, 3, "0.00"); setNumFormat(sheet, row, 5, "0.00"); } const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, sheet, "销售汇总报表"); XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(categorySummary.map((item, i) => ({ 序号: i + 1, 商品分类: item.category, 品规数: item.count, 销量: item.totalSold, 销售金额: item.totalAmount }))), "分类汇总"); XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(paymentSummary.map((item, i) => ({ 序号: i + 1, 收款渠道: item.channel, 月收入: item.amount }))), "收款渠道月统计"); XLSX.writeFile(workbook, `${STORE_NAME}-小商品销售汇总报表.xlsx`); }
  ...
  function exportOnsiteCount() { if (!countRows.length) return alert("本月做到日期当天没有识别到已勾选盘点分类，无法导出现场盘点。"); downloadWorkbook(`${reportProgressDate}-现场盘点表.xlsx`, { 现场盘点: countRows.map((row) => { const actualText = countInput[countKey(row)] ?? ""; const actual = actualText === "" ? "" : n(actualText); const expected = correctedExpectedEndingStock(row); return { 原表行: row.rowNumber, 日期: reportProgressDate, 分类: row.category, 商品名称: row.name, 价格: row.price, 当前应剩数量: expected, 现场实点: actual, 差异: actual === "" ? "" : round2(Number(actual) - expected) }; }) }); }

  return <main className="min-h-screen p-4 md:p-8"><div className="mx-auto max-w-7xl">
    <header className="mb-6 rounded-3xl bg-slate-900 p-6 text-white shadow-sm"><p className="text-sm text-slate-300">Configurable Report Reconciliation</p><h1 className="mt-1 text-2xl font-bold md:text-4xl">报表自动核对助手</h1><p className="mt-2 max-w-3xl text-sm text-slate-300">支持可配置分类、可配置盘点范围、动态收款渠道和原表写回。</p></header>
    <section className="mb-6 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-bold">导入手工报表 Excel</h2><p className="mt-1 text-sm text-slate-500">先配置分类规则，再上传 Excel。系统会按规则识别分类，并按本月做到日期生成现场盘点。</p></div><label className="inline-flex cursor-pointer items-center justify-center rounded-xl bg-slate-900 px-4 py-3 text-sm font-medium text-white">选择报表文件<input className="hidden" type="file" accept=".xlsx,.xls" onChange={(event: ChangeEvent<HTMLInputElement>) => event.target.files?.[0] && handleFile(event.target.files[0])} /></label></div>{fileName ? <p className="mt-3 text-sm text-slate-600">已导入：<b>{fileName}</b>，共解析 {rows.length} 条商品明细，{categorySummary.length} 个分类，{paymentChannels.length} 个收款渠道。本月做到：<b>{reportProgressDate || "未识别"}</b>。</p> : null}</section>
    ...
    {tab === "handover" && rows.length > 0 ? <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-bold">交接和金额异常</h2><p className="mt-1 text-sm text-slate-500">只检查分类规则中勾选“库存核对”的分类。</p></div><button onClick={applyAllWritable} className="rounded-xl bg-yellow-700 px-4 py-2 text-sm text-white">准备写回全部库存异常</button></div><div className="table-scroll mt-4"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{["类型", "分类", "日期", "次日", "商品", "应为", "实际", "差异", "写回位置", "说明", "操作"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{issues.map((item) => { const prepared = item.writeBack ? corrections[item.writeBack.key] : null; return <tr key={item.id} className="border-t"><td className="p-3"><Badge tone={item.type === "金额核对" ? "red" : "yellow"}>{item.type}</Badge></td><td className="p-3">{item.category}</td><td className="p-3">{item.date}</td><td className="p-3">{item.nextDate ?? ""}</td><td className="p-3">{item.name}</td><td className="p-3">{item.expected}</td><td className="p-3">{item.actual}</td><td className="p-3">{item.diff}</td><td className="p-3">{writeBackLocation(item)}</td><td className="p-3">{item.note}</td><td className="p-3">{item.writeBack ? prepared ? <button className="rounded-lg border px-3 py-1 text-xs" onClick={() => undoCorrection(item.writeBack!.key)}>撤销</button> : <button className="rounded-lg bg-slate-900 px-3 py-1 text-xs text-white" onClick={() => applyIssueCorrection(item)}>准备写回</button> : <Badge>只提示</Badge>}</td></tr>; })}</tbody></table></div></section> : null}
    {tab === "count" && rows.length > 0 ? <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-bold">现场盘点清单</h2><p className="mt-1 text-sm text-slate-500">本月做到：{reportProgressDate || "未识别"}。只显示分类规则中勾选“现场盘点”的分类。</p></div><button onClick={exportOnsiteCount} className="rounded-xl border px-4 py-2 text-sm font-medium">导出现场盘点明细</button></div>{!countRows.length ? <div className="mt-4 rounded-xl bg-yellow-50 p-4 text-sm text-yellow-900">本月做到日期是 {reportProgressDate || "未识别"}，但这一天没有识别到已勾选盘点分类。系统不会自动回退到上一天。</div> : null}<div className="table-scroll mt-4"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{["原表行", "分类", "商品", "价格", "当前应剩", "现场实点", "差异", "状态", "纠正原表"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{countRows.map((row) => { const inputKey = countKey(row); const actualText = countInput[inputKey] ?? ""; const actual = actualText === "" ? null : n(actualText); const expected = correctedExpectedEndingStock(row); const diff = actual === null ? null : round2(actual - expected); const correction = corrections[rowCellKey(row, row.nightStockCol)]; return <tr key={rowKey(row)} className="border-t"><td className="p-3 text-slate-500">{row.rowNumber}</td><td className="p-3">{row.category}</td><td className="p-3">{row.name}</td><td className="p-3">¥{money(row.price)}</td><td className="p-3 font-bold">{expected}{correction ? <div className="text-xs font-normal text-slate-500">写回夜班库存：{correction.before} → {correction.after}</div> : null}</td><td className="p-3"><input className="w-28 rounded-xl border p-2" inputMode="decimal" value={actualText} onChange={(event) => setCountInput({ ...countInput, [inputKey]: event.target.value })} /></td><td className="p-3">{diff === null ? "" : diff}</td><td className="p-3">{correction ? <Badge tone="yellow">待写回</Badge> : diff === null ? <Badge>未点</Badge> : diff === 0 ? <Badge tone="green">正常</Badge> : <Badge tone="red">有差异</Badge>}</td><td className="p-3">{actual !== null && diff !== 0 ? <button className="rounded-lg bg-slate-900 px-3 py-1 text-xs text-white" onClick={() => addOnsiteCorrection(row, actual)}>按实点纠正夜班库存</button> : null}{correction ? <button className="ml-2 rounded-lg border px-3 py-1 text-xs" onClick={() => undoCorrection(rowCellKey(row, row.nightStockCol))}>撤销</button> : null}</td></tr>; })}</tbody></table></div>{Object.keys(corrections).length ? <div className="mt-5 rounded-xl bg-yellow-50 p-4 text-sm text-yellow-900"><b>待写回原表 {Object.keys(corrections).length} 条。</b><button className="ml-3 rounded-lg bg-green-700 px-4 py-2 text-sm text-white" onClick={downloadCorrectedOriginal}>下载已纠正原表</button></div> : null}</section> : null}
  </div></main>;
}
