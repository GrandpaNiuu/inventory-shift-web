...
type ProductSummary = { category: string; name: string; price: number; purchase: number; morningSold: number; nightSold: number; totalSold: number; totalAmount: number; endingStock: number; sourceOrder: number };
...
type PaymentSummary = { channel: string; amount: number };
...
type HandoverIssue = { id: string; type: IssueType; date: string; nextDate?: string; category: string; name: string; expected: number; actual: number; diff: number; note: string; writeBack?: WriteBackTarget };
type CorrectionRecord = WriteBackTarget & { diff: number; reason: string; correctedAt: string };
type IssueStats = { totalIssueCount: number; inventoryIssueCount: number; amountIssueCount: number; writableCount: number; preparedCount: number };
type CellWithStyle = XLSX.CellObject & { s?: Record<string, unknown>; z?: string; f?: string; w?: string };
type TableGroup = { headerRow: number; startCol: number; noCol: number; nameCol: number; priceCol: number; purchaseCol: number; morningStockCol: number; morningSoldCol: number; morningAmountCol: number; nightStockCol: number; nightSoldCol: number; nightAmountCol: number; totalAmountCol: number; defaultCategory: string };
type ParsedWorkbook = { rows: StockRow[]; payments: PaymentRecord[] };
...
const HEADER_STYLE = { font: { bold: true, color: { rgb: "FFFFFF" } }, alignment: { horizontal: "center", vertical: "center" }, fill: { fgColor: { rgb: "111827" } } };
const META_STYLE = { font: { bold: true }, fill: { fgColor: { rgb: "F8FAFC" } } };
...
function rowCellKey(row: StockRow, colIndex: number) { return correctionKey(row.sheetName, row.rowIndex, colIndex); }
function countKey(row: StockRow) { return `${row.sheetName}__${row.rowNumber}__${row.category}__${row.name}__${row.price}`; }
function correctedNightStock(row: StockRow) { return row.sameDayExpectedNightStock; }
function isMissedMorningDeduction(row: StockRow) { return row.hasNightStock && row.morningSold > 0 && Math.abs(row.sameDayStockDiff - row.morningSold) < 0.01; }
function byOriginalOrder(a: StockRow, b: StockRow) { return a.sourceOrder - b.sourceOrder || a.rowIndex - b.rowIndex || a.name.localeCompare(b.name); }
...
function groupDefaultCategory(grid: unknown[][], headerRow: number, startCol: number, endCol: number, sheetName: string) {
  ...
  for (let r = 0; r <= headerRow; r += 1) for (let c = startCol; c <= endCol; c += 1) samples.push(text(grid[r]?.[c]));
...
function findTableGroups(grid: unknown[][], sheetName: string) {
  ...
  grid.forEach((row, headerRow) => {
    row.forEach((cell, col) => {
      ...
      seen.add(key);
      groups.push({ headerRow, startCol, noCol: startCol, nameCol: col, priceCol: col + 1, purchaseCol: col + 2, morningStockCol: col + 3, morningSoldCol: col + 4, morningAmountCol: col + 5, nightStockCol: col + 6, nightSoldCol: col + 7, nightAmountCol: col + 8, totalAmountCol: col + 9, defaultCategory: groupDefaultCategory(grid, headerRow, startCol, col + 9, sheetName) });
    });
  });
  return groups;
}
...
function paymentShiftFromContext(grid: unknown[][], rowIndex: number) { const current = rowText(grid[rowIndex]); if (/合计售卖金额|总计|合计/.test(current)) return "合计"; if (/早班/.test(current)) return "早班"; if (/夜班/.test(current)) return "夜班"; const context = `${rowText(grid[rowIndex - 2])} ${rowText(grid[rowIndex - 1])} ${current}`; if (/合计售卖金额|总计|合计/.test(context)) return "合计"; if (/早班/.test(context)) return "早班"; if (/夜班/.test(context)) return "夜班"; return "未知"; }
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    findTableGroups(grid, sheetName).forEach((group) => {
      ...
      for (let i = group.headerRow + 1; i < grid.length; i += 1) {
        ...
        const sourceOrder = sheetIndex * 1_000_000 + group.startCol * 10_000 + i;

        rows.push({ date, sheetName, rowIndex: i, rowNumber: i + 1, sourceOrder, category: inferCategoryFromText(name) ?? currentCategory ?? DEFAULT_CATEGORY, name, price, purchase, morningStock, morningSold, morningAmount, nightStock, nightSold, nightAmount, totalAmount, endingStock, sameDayExpectedNightStock, sameDayStockDiff: hasNightStock ? round2(nightStock - sameDayExpectedNightStock) : 0, morningAmountDiff: hasMorningSold || hasMorningAmount ? round2(morningAmount - morningSold * price) : 0, nightAmountDiff: hasNightSold || hasNightAmount ? round2(nightAmount - nightSold * price) : 0, totalAmountDiff: hasMorningAmount || hasNightAmount || hasTotalAmount ? round2(totalAmount - morningAmount - nightAmount) : 0, hasMorningStock, hasMorningSold, hasNightStock, hasNightSold, hasPurchase, hasMorningAmount, hasNightAmount, hasTotalAmount, morningStockCol: group.morningStockCol, nightStockCol: group.nightStockCol });
      }
    });
  });
  return { rows: rows.sort((a, b) => a.date.localeCompare(b.date) || byOriginalOrder(a, b)), payments: payments.sort((a, b) => `${a.date}-${a.shift}-${a.channel}`.localeCompare(`${b.date}-${b.shift}-${b.channel}`)) };
}
...
function columnName(row: StockRow, colIndex: number) { const col = `${XLSX.utils.encode_col(colIndex)}列`; if (colIndex === row.morningStockCol) return `${col} 早班库存`; if (colIndex === row.nightStockCol) return `${col} 夜班库存`; return col; }
function makeWriteBack(row: StockRow, colIndex: number, before: number, after: number): WriteBackTarget { const cell = XLSX.utils.encode_cell({ r: row.rowIndex, c: colIndex }); return { key: rowCellKey(row, colIndex), date: row.date, sheetName: row.sheetName, rowIndex: row.rowIndex, rowNumber: row.rowNumber, colIndex, cell, columnName: columnName(row, colIndex), category: row.category, name: row.name, price: row.price, before, after: round2(after) }; }
...
function setStyle(sheet: XLSX.WorkSheet, row: number, col: number, style: Record<string, unknown>) { const address = XLSX.utils.encode_cell({ r: row, c: col }); const cell = (sheet[address] ?? { t: "s", v: "" }) as CellWithStyle; cell.s = { ...(cell.s ?? {}), ...style }; sheet[address] = cell; }
function styleRow(sheet: XLSX.WorkSheet, row: number, colCount: number, style: Record<string, unknown>) { for (let col = 0; col < colCount; col += 1) setStyle(sheet, row, col, style); }
...
export default function InventoryPageOrdered() {
  ...
  async function handleFile(file: File) { const buffer = await file.arrayBuffer(); const nextWorkbook = XLSX.read(buffer, { type: "array", cellDates: true, cellFormula: true, cellNF: true, cellStyles: true }); const parsed = parseWorkbook(nextWorkbook); setWorkbook(nextWorkbook); setRows(parsed.rows); setPaymentRecords(parsed.payments); setFileName(file.name); setCountInput({}); setCorrections({}); }

  const daily = useMemo<DailySummary[]>(() => { const map = new Map<string, DailySummary>(); rows.forEach((row) => { const item = map.get(row.date) ?? { date: row.date, purchase: 0, morningSold: 0, nightSold: 0, totalSold: 0, morningAmount: 0, nightAmount: 0, totalAmount: 0, abnormalCount: 0 }; item.purchase += row.purchase; item.morningSold += row.morningSold; item.nightSold += row.nightSold; item.totalSold += row.morningSold + row.nightSold; item.morningAmount += row.morningAmount; item.nightAmount += row.nightAmount; item.totalAmount += row.totalAmount; item.abnormalCount += hasRowIssue(row) ? 1 : 0; map.set(row.date, item); }); return Array.from(map.values()).map((item) => ({ ...item, purchase: round2(item.purchase), morningSold: round2(item.morningSold), nightSold: round2(item.nightSold), totalSold: round2(item.totalSold), morningAmount: round2(item.morningAmount), nightAmount: round2(item.nightAmount), totalAmount: round2(item.totalAmount) })).sort((a, b) => a.date.localeCompare(b.date)); }, [rows]);
  const productSummary = useMemo<ProductSummary[]>(() => { const map = new Map<string, ProductSummary>(); rows.forEach((row) => { const key = `${row.category}__${row.name}__${row.price}`; const item = map.get(key) ?? { category: row.category, name: row.name, price: row.price, purchase: 0, morningSold: 0, nightSold: 0, totalSold: 0, totalAmount: 0, endingStock: 0, sourceOrder: row.sourceOrder }; item.purchase += row.purchase; item.morningSold += row.morningSold; item.nightSold += row.nightSold; item.totalSold += row.morningSold + row.nightSold; item.totalAmount += row.totalAmount; item.endingStock = row.endingStock; item.sourceOrder = Math.min(item.sourceOrder, row.sourceOrder); map.set(key, item); }); return Array.from(map.values()).map((item) => ({ ...item, purchase: round2(item.purchase), morningSold: round2(item.morningSold), nightSold: round2(item.nightSold), totalSold: round2(item.totalSold), totalAmount: round2(item.totalAmount), endingStock: round2(item.endingStock) })).sort((a, b) => a.category.localeCompare(b.category) || a.sourceOrder - b.sourceOrder); }, [rows]);
  ...
  const paymentDaily = useMemo<PaymentDailySummary[]>(() => { const byDate = new Map<string, PaymentRecord[]>(); paymentRecords.forEach((record) => { const list = byDate.get(record.date) ?? []; list.push(record); byDate.set(record.date, list); }); return Array.from(byDate.entries()).map(([date, records]) => { const totalRecords = records.filter((record) => record.shift === "合计"); const source = totalRecords.length ? totalRecords : records.filter((record) => record.shift !== "合计"); const totals = makeEmptyTotals(paymentChannels); source.forEach((record) => { totals[record.channel] = round2((totals[record.channel] ?? 0) + record.amount); }); return { date, totals, total: round2(paymentChannels.reduce((sum, channel) => sum + (totals[channel] ?? 0), 0)) }; }).sort((a, b) => a.date.localeCompare(b.date)); }, [paymentRecords, paymentChannels]);
  const paymentSummary = useMemo<PaymentSummary[]>(() => { const totals = makeEmptyTotals(paymentChannels); paymentDaily.forEach((day) => paymentChannels.forEach((channel) => { totals[channel] = round2((totals[channel] ?? 0) + (day.totals[channel] ?? 0)); })); return paymentChannels.map((channel) => ({ channel, amount: round2(totals[channel] ?? 0) })); }, [paymentDaily, paymentChannels]);

  const handovers = useMemo<HandoverIssue[]>(() => { const issues: HandoverIssue[] = []; rows.forEach((row) => { if (row.hasNightStock && Math.abs(row.sameDayStockDiff) > 0.01) { const type: IssueType = isMissedMorningDeduction(row) ? "疑似未扣早班销量" : "早班交夜班"; const note = type === "疑似未扣早班销量" ? "夜班库存比正确值多出的数量，正好等于早班销量。高度疑似交班时忘记扣早班售卖数量。" : "夜班库存应等于：早班库存 + 进货 - 早班售卖数量。"; const writeBack = makeWriteBack(row, row.nightStockCol, row.nightStock, correctedNightStock(row)); issues.push({ id: `${type}__${writeBack.key}`, type, date: row.date, category: row.category, name: row.name, expected: row.sameDayExpectedNightStock, actual: row.nightStock, diff: row.sameDayStockDiff, note, writeBack }); } if (Math.abs(row.morningAmountDiff) > 0.01) issues.push({ id: `金额核对__早班__${rowKey(row)}`, type: "金额核对", date: row.date, category: row.category, name: row.name, expected: round2(row.morningSold * row.price), actual: row.morningAmount, diff: row.morningAmountDiff, note: "早班金额不等于：早班售卖数量 × 价格。金额核对异常只提示，不自动改原表。" }); if (Math.abs(row.nightAmountDiff) > 0.01) issues.push({ id: `金额核对__夜班__${rowKey(row)}`, type: "金额核对", date: row.date, category: row.category, name: row.name, expected: round2(row.nightSold * row.price), actual: row.nightAmount, diff: row.nightAmountDiff, note: "夜班金额不等于：夜班售卖数量 × 价格。金额核对异常只提示，不自动改原表。" }); if (Math.abs(row.totalAmountDiff) > 0.01) issues.push({ id: `金额核对__合计__${rowKey(row)}`, type: "金额核对", date: row.date, category: row.category, name: row.name, expected: round2(row.morningAmount + row.nightAmount), actual: row.totalAmount, diff: row.totalAmountDiff, note: "合计金额不等于：早班金额 + 夜班金额。金额核对异常只提示，不自动改原表。" }); }); const byProduct = new Map<string, StockRow[]>(); rows.forEach((row) => { const key = `${row.category}__${row.name}__${row.price}`; const list = byProduct.get(key) ?? []; list.push(row); byProduct.set(key, list); }); byProduct.forEach((list) => { const sorted = [...list].sort((a, b) => a.date.localeCompare(b.date)); for (let i = 0; i < sorted.length - 1; i += 1) { const current = sorted[i]; const next = sorted[i + 1]; if (!current.hasNightStock || !next.hasMorningStock) continue; const expected = round2(current.nightStock - current.nightSold); const diff = round2(next.morningStock - expected); if (Math.abs(diff) > 0.01) { const writeBack = makeWriteBack(next, next.morningStockCol, next.morningStock, expected); issues.push({ id: `次日接班__${writeBack.key}__${current.date}`, type: "次日接班", date: current.date, nextDate: next.date, category: current.category, name: current.name, expected, actual: next.morningStock, diff, note: "次日早班库存应等于：前一日夜班库存 - 前一日夜班售卖数量。", writeBack }); } } }); return issues.sort((a, b) => `${a.date}-${a.category}-${a.name}-${a.type}`.localeCompare(`${b.date}-${b.category}-${b.name}-${b.type}`)); }, [rows]);

  const latestDate = daily.length ? daily[daily.length - 1].date : "";
  const latestRows = rows.filter((row) => row.date === latestDate).sort(byOriginalOrder);
  const latestSmokeRows = latestRows.filter((row) => row.category === CIGARETTE_CATEGORY).sort(byOriginalOrder);
  ...
  const stats = useMemo<IssueStats>(() => { const inventoryIssueCount = handovers.filter((item) => item.type !== "金额核对").length; const amountIssueCount = handovers.filter((item) => item.type === "金额核对").length; const writableCount = handovers.filter((item) => item.writeBack).length; return { totalIssueCount: handovers.length, inventoryIssueCount, amountIssueCount, writableCount, preparedCount: correctionRows.length }; }, [handovers, correctionRows.length]);

  function rowKey(row: StockRow) { return `${row.date}__${row.sheetName}__${row.rowNumber}__${row.category}__${row.name}__${row.price}`; }
  function makeCorrection(target: WriteBackTarget, reason: string): CorrectionRecord { return { ...target, after: round2(target.after), diff: round2(target.after - target.before), reason, correctedAt: new Date().toLocaleString("zh-CN", { hour12: false }) }; }
  ...
  function correctedExpectedEndingStock(row: StockRow) { const correction = corrections[rowCellKey(row, row.nightStockCol)]; return correction ? round2(correction.after - row.nightSold) : row.endingStock; }
  function addOnsiteCorrection(row: StockRow, actualEndingStock: number) { const fixedNightStock = round2(actualEndingStock + row.nightSold); const target = makeWriteBack(row, row.nightStockCol, row.nightStock, fixedNightStock); setCorrections((old) => setCorrection(target, "按现场实点反推夜班库存，写回夜班库存列", old)); setCountInput((old) => ({ ...old, [countKey(row)]: String(actualEndingStock) })); }
  ...
  function undoCorrection(row: StockRow) { undoCorrectionByKey(rowCellKey(row, row.nightStockCol)); }
  function applyWritableHandoverCorrections(scope: "all" | "missedMorning") { const writable = handovers.filter((issue) => issue.writeBack && (scope === "all" || issue.type === "疑似未扣早班销量") && !corrections[issue.writeBack.key]); if (!writable.length) return alert("没有新的库存异常需要准备写回。"); if (!confirm(`确定准备写回 ${writable.length} 条库存异常吗？金额核对异常不会自动改原表。`)) return; setCorrections((old) => writable.reduce((acc, issue) => setCorrection(issue.writeBack as WriteBackTarget, issue.note, acc), old)); }
  function downloadCorrectedOriginal() { if (!workbook) return alert("请先上传原始报表 Excel"); const list = Object.values(corrections); if (!list.length) return alert("还没有纠正内容。请先在交接核对或现场盘点里准备写回。"); const fixed = XLSX.read(XLSX.write(workbook, { bookType: "xlsx", type: "array" }), { type: "array", cellDates: true, cellFormula: true, cellNF: true, cellStyles: true }); list.forEach((item) => { const sheet = fixed.Sheets[item.sheetName]; if (sheet) writeCell(sheet, item.rowIndex, item.colIndex, item.after); }); XLSX.writeFile(fixed, `已纠正-${safeFileName(fileName || "手工报表")}.xlsx`); }

  function exportSalesSummaryReport() { if (!rows.length) return alert("请先上传原始报表 Excel"); const totalQty = round2(productSummary.reduce((sum, item) => sum + item.totalSold, 0)); const totalMoney = round2(productSummary.reduce((sum, item) => sum + item.totalAmount, 0)); const grouped = Array.from(productSummary.reduce((map, item) => { const list = map.get(item.category) ?? []; list.push(item); map.set(item.category, list); return map; }, new Map<string, ProductSummary[]>()).entries()).sort(([, a], [, b]) => Math.min(...a.map((item) => item.sourceOrder)) - Math.min(...b.map((item) => item.sourceOrder))); const aoa: Array<Array<string | number>> = [["SY11 小商品销售汇总报表", "", "", "", "", ""], [`门店：${STORE_NAME}`, "", `营业日范围：${reportDateRange(daily)}`, "", `最后查询时间：${queryTime()}`, ""], ["", "", "", "", "", ""], ["序号", "商品分类", "商品名称", "平均价格", "数量", "金额"]]; const categoryRows: number[] = []; const subtotalRows: number[] = []; let index = 1; grouped.forEach(([category, products]) => { categoryRows.push(aoa.length); aoa.push([`【${category}】`, "", "", "", "", ""]); products.sort((a, b) => a.sourceOrder - b.sourceOrder).forEach((item) => { aoa.push([index, item.category, item.name, round2(item.price), round2(item.totalSold), round2(item.totalAmount)]); index += 1; }); subtotalRows.push(aoa.length); aoa.push([`${category}小计`, "", "", "", round2(products.reduce((sum, item) => sum + item.totalSold, 0)), round2(products.reduce((sum, item) => sum + item.totalAmount, 0))]); aoa.push(["", "", "", "", "", ""]); }); const totalRow = aoa.length; aoa.push(["全部总计", "", "", "", totalQty, totalMoney]); const sheet = XLSX.utils.aoa_to_sheet(aoa); sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 1 } }, { s: { r: 1, c: 2 }, e: { r: 1, c: 3 } }, { s: { r: 1, c: 4 }, e: { r: 1, c: 5 } }, ...categoryRows.map((row) => ({ s: { r: row, c: 0 }, e: { r: row, c: 5 } }))]; sheet["!cols"] = [{ wch: 10 }, { wch: 20 }, { wch: 34 }, { wch: 12 }, { wch: 12 }, { wch: 14 }]; styleRow(sheet, 0, 6, TITLE_STYLE); styleRow(sheet, 1, 6, META_STYLE); styleRow(sheet, 3, 6, HEADER_STYLE); categoryRows.forEach((row) => styleRow(sheet, row, 6, CATEGORY_STYLE)); subtotalRows.forEach((row) => styleRow(sheet, row, 6, SUBTOTAL_STYLE)); styleRow(sheet, totalRow, 6, TOTAL_STYLE); for (let row = 4; row <= totalRow; row += 1) { setNumFormat(sheet, row, 3, "0.00"); setNumFormat(sheet, row, 5, "0.00"); } const categorySheet = XLSX.utils.json_to_sheet([...categorySummary.map((item, i) => ({ 序号: i + 1, 商品分类: item.category, 品规数: item.count, 销售金额: round2(item.totalAmount) })), { 序号: "全部总计", 商品分类: "", 品规数: categorySummary.reduce((sum, item) => sum + item.count, 0), 销售金额: round2(totalMoney) }]); categorySheet["!cols"] = [{ wch: 10 }, { wch: 22 }, { wch: 12 }, { wch: 14 }]; const paymentSheet = XLSX.utils.json_to_sheet([...paymentSummary.map((item, i) => ({ 序号: i + 1, 收款渠道: item.channel, 月收入: item.amount })), { 序号: "总计", 收款渠道: "", 月收入: round2(totalPaymentAmount) }]); paymentSheet["!cols"] = [{ wch: 10 }, { wch: 18 }, { wch: 14 }]; const reportWorkbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(reportWorkbook, sheet, "销售汇总报表"); XLSX.utils.book_append_sheet(reportWorkbook, categorySheet, "分类汇总"); XLSX.utils.book_append_sheet(reportWorkbook, paymentSheet, "收款渠道月统计"); XLSX.writeFile(reportWorkbook, `${STORE_NAME}-小商品销售汇总报表.xlsx`); }
  function exportExceptionReport() { if (!rows.length) return alert("请先上传原始报表 Excel"); const aoa: Array<Array<string | number>> = [["报表异常核对报表", "", "", "", "", "", "", "", "", "", ""], [`门店：${STORE_NAME}`, `日期范围：${reportDateRange(daily)}`, `总异常数：${stats.totalIssueCount}`, `库存异常数：${stats.inventoryIssueCount}`, `金额异常数：${stats.amountIssueCount}`, "", "", "", "", "", ""], ["", "", "", "", "", "", "", "", "", "", ""], ["序号", "异常类型", "商品分类", "日期", "次日", "商品名称", "应为", "实际", "差异", "写回位置", "说明"], ...handovers.map((item, i) => [i + 1, item.type, item.category, item.date, item.nextDate ?? "", item.name, item.expected, item.actual, item.diff, writeBackLocation(item), item.note]), ["总计", `共 ${stats.totalIssueCount} 条`, "", "", "", `库存异常 ${stats.inventoryIssueCount} 条`, `金额异常 ${stats.amountIssueCount} 条`, "", "", `可写回 ${stats.writableCount} 条`, ""]]; const sheet = XLSX.utils.aoa_to_sheet(aoa); sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 10 } }]; sheet["!cols"] = [{ wch: 8 }, { wch: 18 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 28 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 24 }, { wch: 52 }]; const reportWorkbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(reportWorkbook, sheet, "异常核对报表"); XLSX.writeFile(reportWorkbook, "报表异常核对报表.xlsx"); }
  function exportOnsiteCount() { if (!latestSmokeRows.length) return alert("最新日期没有香烟明细，现场盘点只显示香烟。"); const countRows = latestSmokeRows.map((row) => { const actualText = countInput[countKey(row)] ?? ""; const actual = actualText === "" ? "" : n(actualText); const expected = correctedExpectedEndingStock(row); const diff = actual === "" ? "" : round2(Number(actual) - expected); const correction = corrections[rowCellKey(row, row.nightStockCol)]; return { 原表序号: row.rowNumber, 日期: latestDate, 商品分类: row.category, 商品名称: row.name, 价格: row.price, 原应剩数量: row.endingStock, 当前应剩数量: expected, 现场实点: actual, 差异: diff, 是否已准备写回: correction ? "是" : "否", 写回位置: correction ? `${correction.sheetName}!${correction.cell}` : "" }; }); downloadWorkbook(`${latestDate}-香烟现场盘点表.xlsx`, { 香烟现场盘点: countRows, 差异香烟: countRows.filter((row) => row.差异 !== "" && row.差异 !== 0) }); }
