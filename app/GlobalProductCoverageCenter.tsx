...
function colLabel(index: number) { return index >= 0 ? XLSX.utils.encode_col(index) : "未识别"; }
...
function exportCoverage(fileName: string, rows: ProductRecord[]) {
  ...
  XLSX.writeFile(workbook, `${fileName.replace(/\.xlsx?$/i, "")}-全球商品覆盖报告.xlsx`);
}
...
export default function GlobalProductCoverageCenter() {
  ...
  return <section className="space-y-6">
    <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-bold">全球商品覆盖中心</h2><p className="mt-1 text-sm text-slate-500">上传任何包含商品名称的 Excel，系统会按全球商品词库自动识别分类、覆盖率和未识别商品。不再只覆盖香烟和饮料。</p></div><label className="inline-flex cursor-pointer items-center justify-center rounded-xl bg-slate-950 px-4 py-3 text-sm font-medium text-white">选择商品表<input className="hidden" type="file" accept=".xlsx,.xls" onChange={(event: ChangeEvent<HTMLInputElement>) => event.target.files?.[0] && handleFile(event.target.files[0])} /></label></div>{fileName ? <p className="mt-3 text-sm text-slate-600">已导入：<b>{fileName}</b>，识别 {sheets.length} 个 Sheet，{rows.length} 个商品行。</p> : null}</div>
    ...
    {unmatchedRows.length ? <div className="rounded-2xl border bg-yellow-50 p-5 text-yellow-900 shadow-sm"><h3 className="font-bold">待补充商品</h3><p className="mt-1 text-sm">这些商品没有高置信度匹配。后续可把它们加入商品别名库或全球商品词库，系统会越用越准。</p><div className="mt-3 flex flex-wrap gap-2">{unmatchedRows.slice(0, 80).map((row) => <span key={`${row.sheetName}-${row.rowNumber}`} className="rounded-full bg-white px-3 py-1 text-xs shadow-sm">{row.productName}</span>)}</div></div> : null}</> : null}
  </section>;
}
