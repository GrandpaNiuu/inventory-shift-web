...
type ProductSummary = { category: string; name: string; price: number; purchase: number; morningSold: number; nightSold: number; totalSold: number; totalAmount: number; endingStock: number };
type CategorySummary = { category: string; totalSold: number; totalAmount: number; count: number };
...
function rowKey(row: StockRow, date = row.date) { return `${date}__${row.sheetName}__${row.rowNumber}__${row.category}__${row.name}__${row.price}`; }
function countKey(row: StockRow) { return `${row.category}__${row.name}__${row.price}`; }
...
function paymentShiftFromContext(grid: unknown[][], rowIndex: number) { const current = rowText(grid[rowIndex]); if (/合计售卖金额|总计|合计/.test(current)) return "合计" as PaymentShift; if (/早班/.test(current)) return "早班" as PaymentShift; if (/夜班/.test(current)) return "夜班" as PaymentShift; const context = `${rowText(grid[rowIndex - 2])} ${rowText(grid[rowIndex - 1])} ${current}`; if (/合计售卖金额|总计|合计/.test(context)) return "合计" as PaymentShift; if (/早班/.test(context)) return "早班" as PaymentShift; if (/夜班/.test(context)) return "夜班" as PaymentShift; return "未知" as PaymentShift; }
...
function parseWorkbook(workbook: XLSX.WorkBook): ParsedWorkbook {
  ...
  return { rows: parsedRows.sort((a, b) => `${a.date}-${a.category}-${a.rowNumber}`.localeCompare(`${b.date}-${b.category}-${b.rowNumber}`)), payments: payments.sort((a, b) => `${a.date}-${a.shift}-${a.channel}`.localeCompare(`${b.date}-${b.shift}-${b.channel}`)) };
}
...
function Card({ title, value, desc, tone = "default" }: { title: string; value: string; desc?: string; tone?: "default" | "danger" }) { const cls = tone === "danger" ? "border-red-500 bg-red-600 text-white shadow-sm" : "border bg-white text-slate-900 shadow-sm"; const titleCls = tone === "danger" ? "text-red-100" : "text-slate-500"; const descCls = tone === "danger" ? "text-red-100" : "text-slate-500"; return <div className={`rounded-2xl p-5 ${cls}`}><div className={`text-sm ${titleCls}`}>{title}</div><div className="mt-2 text-2xl font-bold">{value}</div>{desc ? <div className={`mt-1 text-xs ${descCls}`}>{desc}</div> : null}</div>; }
...
export default function InventoryPage() {
  ...
  const productSummary = useMemo<ProductSummary[]>(() => { const map = new Map<string, ProductSummary>(); rows.forEach((row) => { const key = `${row.category}__${row.name}__${row.price}`; const item = map.get(key) ?? { category: row.category, name: row.name, price: row.price, purchase: 0, morningSold: 0, nightSold: 0, totalSold: 0, totalAmount: 0, endingStock: 0 }; item.purchase += row.purchase; item.morningSold += row.morningSold; item.nightSold += row.nightSold; item.totalSold += row.morningSold + row.nightSold; item.totalAmount += row.totalAmount; item.endingStock = row.endingStock; map.set(key, item); }); return Array.from(map.values()).map((item) => ({ ...item, purchase: round2(item.purchase), morningSold: round2(item.morningSold), nightSold: round2(item.nightSold), totalSold: round2(item.totalSold), totalAmount: round2(item.totalAmount), endingStock: round2(item.endingStock) })).sort((a, b) => a.category.localeCompare(b.category) || b.totalAmount - a.totalAmount); }, [rows]);

  const categorySummary = useMemo<CategorySummary[]>(() => { const map = new Map<string, CategorySummary>(); productSummary.forEach((item) => { const current = map.get(item.category) ?? { category: item.category, totalSold: 0, totalAmount: 0, count: 0 }; current.totalSold += item.totalSold; current.totalAmount += item.totalAmount; current.count += 1; map.set(item.category, current); }); return Array.from(map.values()).map((item) => ({ ...item, totalSold: round2(item.totalSold), totalAmount: round2(item.totalAmount) })).sort((a, b) => b.totalAmount - a.totalAmount); }, [productSummary]);
  ...
  const paymentDaily = useMemo<PaymentDailySummary[]>(() => { const byDate = new Map<string, PaymentRecord[]>(); paymentRecords.forEach((record) => { const list = byDate.get(record.date) ?? []; list.push(record); byDate.set(record.date, list); }); return Array.from(byDate.entries()).map(([date, records]) => { const totalRecords = records.filter((record) => record.shift === "合计"); const source = totalRecords.length ? totalRecords : records.filter((record) => record.shift !== "合计"); const totals = makeEmptyTotals(paymentChannels); source.forEach((record) => { totals[record.channel] = round2((totals[record.channel] ?? 0) + record.amount); }); const total = round2(paymentChannels.reduce((sum, channel) => sum + (totals[channel] ?? 0), 0)); return { date, totals, total }; }).sort((a, b) => a.date.localeCompare(b.date)); }, [paymentRecords, paymentChannels]);
  ...
  function downloadCorrectedOriginal() { if (!workbook) return alert("请先上传原始报表 Excel"); const correctionList = Object.values(corrections); if (!correctionList.length) return alert("还没有纠正内容。请先在交接核对或现场盘点里准备写回。"); const fixed = XLSX.read(XLSX.write(workbook, { bookType: "xlsx", type: "array" }), { type: "array", cellDates: true, cellFormula: true, cellNF: true, cellStyles: true }); correctionList.forEach((item) => { const sheet = fixed.Sheets[item.sheetName]; if (sheet) writeCell(sheet, item.rowIndex, item.colIndex, item.after); }); XLSX.writeFile(fixed, `已纠正-${safeFileName(fileName || "手工报表")}.xlsx`); }

  function exportSalesSummaryReport() { if (!rows.length) return alert("请先上传原始报表 Excel"); const totalQty = round2(productSummary.reduce((sum, item) => sum + item.totalSold, 0)); const totalMoney = round2(productSummary.reduce((sum, item) => sum + item.totalAmount, 0)); const aoa: Array<Array<string | number>> = [["SY11 小商品销售汇总报表", "", "", "", "", ""], [`门店：${STORE_NAME}`, "", `营业日范围：${reportDateRange(daily)}`, "", `最后查询时间：${queryTime()}`, ""], ["", "", "", "", "", ""], ["序号", "商品分类", "商品名称", "平均价格", "数量", "金额"], ...productSummary.map((item, index) => [index + 1, item.category, item.name, round2(item.price), round2(item.totalSold), round2(item.totalAmount)]), ["总计", "", "", "", totalQty, totalMoney]]; const sheet = XLSX.utils.aoa_to_sheet(aoa); const totalRow = aoa.length - 1; sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 1 } }, { s: { r: 1, c: 2 }, e: { r: 1, c: 3 } }, { s: { r: 1, c: 4 }, e: { r: 1, c: 5 } }]; sheet["!cols"] = [{ wch: 8 }, { wch: 20 }, { wch: 32 }, { wch: 12 }, { wch: 12 }, { wch: 14 }]; setCellStyle(sheet, 0, 0, TITLE_STYLE); styleRow(sheet, 3, 6, HEADER_STYLE); styleRow(sheet, totalRow, 6, TOTAL_STYLE); for (let row = 4; row <= totalRow; row += 1) { setNumFormat(sheet, row, 3, "0.00"); setNumFormat(sheet, row, 5, "0.00"); } const paymentRows = [...paymentSummary.map((item, index) => ({ 序号: index + 1, 收款渠道: item.channel, 月收入: item.amount })), { 序号: "总计", 收款渠道: "", 月收入: round2(totalPaymentAmount) }]; const paymentSheet = XLSX.utils.json_to_sheet(paymentRows); paymentSheet["!cols"] = [{ wch: 8 }, { wch: 18 }, { wch: 14 }]; const reportWorkbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(reportWorkbook, sheet, "销售汇总报表"); XLSX.utils.book_append_sheet(reportWorkbook, paymentSheet, "收款渠道月统计"); XLSX.writeFile(reportWorkbook, `${STORE_NAME}-小商品销售汇总报表.xlsx`); }
  function exportExceptionReport() { if (!rows.length) return alert("请先上传原始报表 Excel"); const aoa: Array<Array<string | number>> = [["报表异常核对报表", "", "", "", "", "", "", "", "", "", ""], [`门店：${STORE_NAME}`, `日期范围：${reportDateRange(daily)}`, `总异常数：${stats.totalIssueCount}`, `库存异常数：${stats.inventoryIssueCount}`, `金额异常数：${stats.amountIssueCount}`, "", "", "", "", "", ""], ["", "", "", "", "", "", "", "", "", "", ""], ["序号", "异常类型", "商品分类", "日期", "次日", "商品名称", "应为", "实际", "差异", "写回位置", "说明"], ...handovers.map((item, index) => [index + 1, item.type, item.category, item.date, item.nextDate ?? "", item.name, item.expected, item.actual, item.diff, writeBackLocation(item), item.note]), ["总计", `共 ${stats.totalIssueCount} 条`, "", "", "", `库存异常 ${stats.inventoryIssueCount} 条`, `金额异常 ${stats.amountIssueCount} 条`, "", "", `可写回 ${stats.writableCount} 条`, ""]]; const sheet = XLSX.utils.aoa_to_sheet(aoa); const totalRow = aoa.length - 1; sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 10 } }]; sheet["!cols"] = [{ wch: 8 }, { wch: 18 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 28 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 24 }, { wch: 52 }]; setCellStyle(sheet, 0, 0, TITLE_STYLE); styleRow(sheet, 3, 11, HEADER_STYLE); styleRow(sheet, totalRow, 11, TOTAL_STYLE); const reportWorkbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(reportWorkbook, sheet, "异常核对报表"); XLSX.writeFile(reportWorkbook, "报表异常核对报表.xlsx"); }
  function exportOnsiteCount() { if (!latestSmokeRows.length) return alert("最新日期没有香烟明细，现场盘点只显示香烟。"); const countRows = latestSmokeRows.map((row) => { const actualText = countInput[countKey(row)] ?? ""; const actual = actualText === "" ? "" : n(actualText); const expected = correctedExpectedEndingStock(row); const diff = actual === "" ? "" : round2(Number(actual) - expected); const correction = corrections[rowCellKey(row, row.nightStockCol)]; return { 日期: latestDate, 商品分类: row.category, 商品名称: row.name, 价格: row.price, 原应剩数量: row.endingStock, 当前应剩数量: expected, 现场实点: actual, 差异: diff, 是否已准备写回: correction ? "是" : "否", 写回位置: correction ? `${correction.sheetName}!${correction.cell}` : "" }; }); downloadWorkbook(`${latestDate}-香烟现场盘点表.xlsx`, { 香烟现场盘点: countRows, 差异香烟: countRows.filter((row) => row.差异 !== "" && row.差异 !== 0) }); }
