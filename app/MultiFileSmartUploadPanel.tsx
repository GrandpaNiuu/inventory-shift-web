...
type PayrollRow = { employee: string; department: string; gross: number; net: number; calculatedGross: number; calculatedNet: number };
...
function cleanChannelName(value: unknown) { const channel = compact(value); if (!channel) return ""; if (/^(金额|收款金额|合计|总计|早班|夜班|收款渠道|渠道|售卖金额)$/.test(channel)) return ""; if (/^\d+(\.\d+)?$/.test(channel)) return ""; return channel; }
...
function paymentShiftFromContext(grid: unknown[][], rowIndex: number): PaymentShift {
  for (let r = rowIndex; r >= Math.max(0, rowIndex - 8); r -= 1) {
    const line = compact(rowText(grid[r]));
    if (!line) continue;
    if (/早班/.test(line)) return "早班";
    if (/夜班/.test(line)) return "夜班";
    if (/^(合计售卖金额|总计售卖金额|总计|合计)/.test(line)) return "合计";
  }
  return "未知";
}
...
function paymentSourceForReconcile(payments: PaymentRecord[]) {
  ...
  return Array.from(byFileDate.values()).flatMap((records) => {
    ...
    return records.filter((record) => record.shift === "早班" || record.shift === "夜班" || record.shift === "未知");
  });
}
...
function channelSummary(payments: PaymentRecord[]) {
  ...
  paymentSourceForReconcile(payments).forEach((record) => {
    ...
    old.amount = round2(old.amount + record.amount);
    old.count += 1;
...
function findProductGroups(grid: unknown[][]) {
  ...
  grid.forEach((row, headerRow) => row.forEach((cell, col) => {
    const label = compact(cell);
    const next = compact(row[col + 1]);
...
function result(fileName: string, storeName: string, kind: BatchKind, rowCount: number, primaryAmount: number, secondaryAmount: number, diffAmount: number, issues: BatchIssue[], payments: PaymentRecord[] = []): BatchResult {
  return { fileName, storeName, kind, rowCount, primaryAmount: round2(primaryAmount), secondaryAmount: round2(secondaryAmount), diffAmount: round2(diffAmount), issueCount: issues.length, channelCount: channelSummary(payments).length, issues, payments };
}
...
function parseProductFile(workbook: XLSX.WorkBook, fileName: string, storeName: string): BatchResult | null {
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    if (!shouldProcessSheet(sheetName, grid)) return;
    ...
    findProductGroups(grid).forEach((group) => {
      for (let r = group.headerRow + 1; r < grid.length; r += 1) {
        ...
        if (!name || !price || /合计|总计|小计|备注|收款|金额|售卖数量|售卖金额|渠道/.test(name)) continue;
...
function findHeaderIndex(headers: unknown[], names: string[]) {
  return headers.findIndex((header) => names.some((name) => compact(header).includes(compact(name))));
}

function parsePayrollOrGenericFile(workbook: XLSX.WorkBook, fileName: string, storeName: string): BatchResult {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    if (!sheet) return;
    const grid = XLSX.utils.sheet_to_json<unknown[]>(sheet, { header: 1, defval: null });
    const headerRow = grid.findIndex((row) => row.some((cell) => /员工|姓名|应发|实发|工资|金额|收入/.test(text(cell))));
    if (headerRow < 0) return;
    const headers = grid[headerRow] ?? [];
    const employeeCol = findHeaderIndex(headers, ["员工", "姓名", "员工姓名"]);
    const departmentCol = findHeaderIndex(headers, ["部门", "门店", "班组"]);
    const grossCol = findHeaderIndex(headers, ["应发", "应发工资", "工资合计"]);
    const netCol = findHeaderIndex(headers, ["实发", "实发工资", "实际发放"]);
    const amountCol = findHeaderIndex(headers, ["金额", "收入", "销售额", "实收"]);

    for (let r = headerRow + 1; r < grid.length; r += 1) {
      const row = grid[r] ?? [];
      if (!row.some((cell) => text(cell))) continue;
      if (employeeCol >= 0 || grossCol >= 0 || netCol >= 0) {
        ...
        if (!employee && !gross && !net) continue;
        payrollRows.push({ employee, department: departmentCol >= 0 ? text(row[departmentCol]) : "", gross, net, calculatedGross: gross, calculatedNet: net });
      } else if (amountCol >= 0) {
        genericAmount += toNumber(row[amountCol]);
      }
    }
  });
  ...
  if (payrollRows.length) {
    ...
    return result(fileName, storeName, "payroll", payrollRows.length, gross, net, 0, issues);
  }
  return result(fileName, storeName, "generic", genericAmount ? 1 : 0, genericAmount, 0, 0, issues);
}
...
async function analyzeFile(file: File): Promise<BatchResult> {
  ...
  return parseProductFile(workbook, fileName, storeName) ?? parsePayrollOrGenericFile(workbook, fileName, storeName);
}

function exportBatch(results: BatchResult[]) {
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(results.map((item, index) => ({ 序号: index + 1, 门店: item.storeName, 文件: item.fileName, 类型: item.kind === "product" ? "商品表" : item.kind === "payroll" ? "工资表" : "通用表", 数据行: item.rowCount, 销售或应发: item.primaryAmount, 收款或实发: item.secondaryAmount, 核对差异: item.diffAmount, 渠道数: item.channelCount, 异常数: item.issueCount }))), "全部门店总览");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(results.flatMap((item) => paymentSourceForReconcile(item.payments)).map((record, index) => ({ 序号: index + 1, 门店: record.storeName, 文件: record.fileName, 日期: record.date, Sheet: record.sheetName, 班次: record.shift, 收款渠道: record.channel, 金额: record.amount }))), "有效收款明细");
  ...
  XLSX.writeFile(wb, `智能核对报告-${new Date().toISOString().slice(0, 10)}.xlsx`);
}
...
export default function MultiFileSmartUploadPanel() {
  ...
  async function handleFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    setLoading(true);
    try { setResults(await Promise.all(files.map(analyzeFile))); }
  ...
  const validChannels = channelSummary(results.flatMap((item) => item.payments));

  return <section className="mx-auto mb-6 max-w-7xl px-4 md:px-8">
    <div className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div><h2 className="text-lg font-bold text-slate-900">统一自动识别上传</h2><p className="mt-1 text-sm text-slate-500">修复规则：白班/夜班/总合计同时存在时，只用总合计；没有总合计才用白班 + 夜班。</p></div>
        <label className="inline-flex cursor-pointer items-center justify-center rounded-xl bg-slate-950 px-4 py-3 text-sm font-medium text-white">{loading ? "处理中..." : "选择 Excel"}<input className="hidden" type="file" accept=".xlsx,.xls" multiple onChange={handleFiles} /></label>
      </div>
      {results.length ? <>
        <div className="mt-5 grid gap-4 md:grid-cols-5">
          <Card label="文件数" value={String(results.length)} />
          <Card label="数据行" value={String(totalRows)} />
          <Card label="销售/应发" value={`¥${money(totalPrimary)}`} />
          <Card label="收款/实发" value={`¥${money(totalSecondary)}`} />
          <Card label="核对差异" value={`¥${money(totalDiff)}`} danger={Math.abs(totalDiff) > 0.01 || totalIssues > 0} />
        </div>
        ...
        <div className="mt-5 rounded-xl bg-slate-50 p-4"><div className="font-bold text-slate-900">有效收款渠道汇总</div><div className="mt-3 grid gap-2 md:grid-cols-3">{validChannels.map((item) => <div key={item.channel} className="rounded-xl bg-white p-3 text-sm shadow-sm"><div className="font-medium">{item.channel}</div><div className="mt-1 text-lg font-bold">¥{money(item.amount)}</div><div className="text-xs text-slate-500">{item.count} 条有效记录</div></div>)}</div></div>
        ...
        <button onClick={() => exportBatch(results)} className="mt-5 rounded-xl bg-green-700 px-4 py-3 text-sm font-medium text-white">导出总报告</button>
      </> : null}
    </div>
  </section>;
}

function Card({ label, value, danger = false }: { label: string; value: string; danger?: boolean }) {
  return <div className={`rounded-xl p-4 ${danger ? "bg-red-600 text-white" : "bg-slate-50 text-slate-900"}`}><div className="text-sm opacity-80">{label}</div><div className="mt-1 text-2xl font-bold">{value}</div></div>;
}
